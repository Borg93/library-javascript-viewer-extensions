window["_Viewing.Extension.ControlSelector"] =
...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/Viewing.Extension.ControlSelector/Viewing.Extension.ControlSelector.js");
/******/ })
/************************************************************************/
/******/ ({
...
Viewing.Extension.ControlSelector = function (viewer, options) {
  ...
  _thisExtension.load = function () {
    var buttonId = guid();
    _panel = new Panel(viewer.container, guid(), buttonId);
    ...
    if (options.Lockr) {
      ...
      if (!ControlSelector) {
        ...
        options.Lockr.set(options.storageKey, ControlSelector);
      }
  ...
  function createToolbar(buttonId) {
    var toolbar = new Autodesk.Viewing.UI.ToolBar('viewing-extension-ControlSelector-toolbar');
    var ctrlGroup = new Autodesk.Viewing.UI.ControlGroup('viewing-extension-ControlSelector-controlgroup');
    var button = createButton(buttonId, 'glyphicon glyphicon-cog', 'UI Settings', togglePanel);
  ...
  function loadFromStorage() {
    ...
    $('.adsk-control-group').each(function () {
      if (!$(this).hasClass('adsk-hidden') && this.id !== 'viewing-extension-ControlSelector-controlgroup') {
  ...
  var Panel = function Panel(parentContainer, id, controlId) {
    var _thisPanel = this;

    Autodesk.Viewing.UI.DockingPanel.call(this, parentContainer, id, 'Select Visible Controls', {
      shadow: true
    });
    ...
    var html = ['<div class="ui-settings-panel-content">', '<ul id="' + id + '-item-list" class="ui-settings-item-list">', '</ul>', '<div>'];
    ...
    function clearButtons() {
      $('#' + id + '-item-list > li').each(function (idx, child) {
        $(child).remove();
      });
    } /////////////////////////////////////////////
    //
    ...
    function addItem(button) {
      ...
      var item = ['<li class="list-group-item ui-settings-item" id="' + itemId + '">', text, '</li>'].join('\n');
      $('#' + id + '-item-list').append(item);
      if ($button.css('display') !== 'none') $('#' + itemId).addClass('enabled');
  ...
  var css = ['div.ui-settings-panel-content{', 'height: calc(100% - 40px);', 'overflow-y: scroll;', '}', 'ul.ui-settings-item-list{', 'padding: 0;', 'margin:5px', '}', 'li.ui-settings-item{', 'color: #FFFFFF;', 'background-color: #3F4244;', 'list-style-type: none;', 'margin-bottom:  5px;', 'border-radius:4px', '}', 'li.ui-settings-item:hover{', 'cursor: pointer;', 'color: #FFFFFF;', 'background-color: #5BC0DE;', '}', 'li.ui-settings-item.enabled {', 'color: #000000;', 'background-color: #00CC00;', '}'].join('\n');
  $('<style type="text/css">' + css + '</style>').appendTo('head');
};

Viewing.Extension.ControlSelector.prototype = Object.create(Autodesk.Viewing.Extension.prototype);
Viewing.Extension.ControlSelector.prototype.constructor = Viewing.Extension.ControlSelector;
Autodesk.Viewing.theExtensionManager.registerExtension('_Viewing.Extension.ControlSelector', Viewing.Extension.ControlSelector);
